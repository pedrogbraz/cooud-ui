    ...
    <Menubar>
      <MenubarMenu>
        ...
        <MenubarContent>
          ...
          <MenubarSeparator />
          <MenubarItem onSelect={onEdit}>Edit</MenubarItem>
          <MenubarItem onSelect={onDelete}>Delete</MenubarItem>
        </MenubarContent>
      </MenubarMenu>
    </Menubar>
  );
...
describe("Menubar", () => {
  ...
  it("runs an item's handler and closes when the item is activated by click", async () => {
    const onEdit = vi.fn();
    const user = userEvent.setup();
    render(<BasicMenubar onEdit={onEdit} />);
    await user.click(screen.getByRole("menuitem", { name: "File" }));
    const menu = await screen.findByRole("menu");
  ...
  it("activates the highlighted item with Enter and closes", async () => {
    ...
    const user = userEvent.setup();
    render(<BasicMenubar onDelete={onDelete} />);
    await user.click(screen.getByRole("menuitem", { name: "File" }));
    await screen.findByRole("menu");
  ...
  it("has no axe violations while open", async () => {
    const user = userEvent.setup();
    const { baseElement } = render(<BasicMenubar />);
    await user.click(screen.getByRole("menuitem", { name: "File" }));
    await screen.findByRole("menu");
