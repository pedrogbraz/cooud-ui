/**
 * `cooud-ui add-page` — incrementally add ONE page to an already-composed app
...
import { loadManifestFile, loadTemplate } from "./compose.js";
...
export interface AddPageOptions {
  /** Project root (must contain cooud-ui.json with a composed app). */
  targetDir: string;
...
function pickComposedApp(
  ...
  if (keys.length === 0) {
    throw new Error("No composed app found. Run `cooud-ui compose <template>` first.");
  }
 ...
 * `loadTemplate` silently returns the WRONG (bundled) manifest, and synthesizeManifest
...
export async function addPage(options: AddPageOptions): Promise<AddPageResult> {
  ...
  // nav const + the "Cooud" brand literal, so it MUST be customized to match what
  ...
  // compose does): the brand literal ("Cooud") only exists in the pristine source
...
/**
 * The `cooud-ui add-page` command: adds ONE page to a composed app, or previews
...
export async function addPageCommand(options: AddPageCommandOptions): Promise<void> {
  ...
  if (options.route === undefined || options.route.length === 0) {
    log.err("--route is required, e.g. `cooud-ui add-page --route /faq --blocks faq,cta`.");
    process.exitCode = 1;
    return;
  }
