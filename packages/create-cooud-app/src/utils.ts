...
/**
 * The template names generated by the Cooud Compose app generator (no bundled
 * dir): they scaffold the `default` base, then `composeApp()` from the bundled
 * `store`/`landing` manifest renders the pages + chrome from validated blocks.
 * The value is the manifest/template name passed to `composeApp`.
 */
 ...
 * Composed templates (`store`/`landing`) reuse the `default` base; the others
 * are their own name.
 */
